...
def test_content(client, user_token):
    ...
    filename = (
        ...
        + str(today.month)
        + "/"
        + "06316e26031f7c69c9cb7de53541e8e0.png"
    )
    ...
    rv = client.post(
        "/content/",
        data=dict(file=(open("testing-data/1.png", "rb"), "testing-data/1.png"),),
    ...
    assert os.path.getsize("testing-data/1.png") == os.path.getsize(
        "uploads/" + filename
    ), "There is a problem with image uploading"

    rv = client.get("/content/" + str(rv.json["file"]["id"]) + "/")
    assert len(rv.data) == os.path.getsize(
        "testing-data/1.png"
