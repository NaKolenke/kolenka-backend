...
def validate_tokens(json):
    ...
    access_token = (
        ...
        .where(
            ...
            & (Token.token == json["access_token"]["token"])
        )
        .get()
    )
    assert (
    ...
    refresh_token = (
        ...
        .where(
            ...
            & (Token.token == json["refresh_token"]["token"])
        )
        .get()
    )
    assert (
...
def test_token_valid(client, tokens):
    rv = client.post("/token/validate/", json={"token": tokens["access_token"].token})
    assert rv.status_code == 200
    assert rv.json["success"] == 1


def test_token_invalid(client, tokens):
    rv = client.post("/token/validate/", json={"token": "0"})
...
def test_refresh_token(client, tokens):
    rv = client.post("/token/refresh/", json={"token": tokens["refresh_token"].token})
    assert rv.status_code == 200
    assert rv.json["success"] == 1
...
def test_refresh_token_invalid(client, tokens):
    rv = client.post("/token/refresh/", json={"token": "0"})
    assert rv.status_code == 400
    assert rv.json["success"] == 0
    assert rv.json["error"]["code"] == 10
...
def test_self(client, tokens):
    headers = {"Authorization": tokens["access_token"].token}
