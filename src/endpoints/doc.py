...
def documentation():
    ...
    for rule in current_app.url_map.iter_rules():
        ...
        method = [item for item in rule.methods if item not in ["HEAD", "OPTIONS"]]
        ...
        endpoints.append(Endpoint(method, url, section, doc, body, response))
    ...
    for (name, func) in functions:
        ...
        e = {"response_code": actual_error.status_code, "error": response_body["error"]}
        errors.append(e)
    ...
    return jsonify(
        {"success": 1, "endpoints": [e.__dict__ for e in endpoints], "errors": errors}
    )
